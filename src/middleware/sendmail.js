...
exports.sendmail = (email, token, status) => {
  ...
  hostwifi = '3.1.148.52:3499/'
  //hostwifi = '192.168.1.119:3499/'
  sendto = email;

  encry = crypto.encryto("email=" + sendto)
  let encry_forGetpass = crypto.encryto("email=" + sendto + "&token=" + token)
  ...
  let link = (status == 1 ? link_regis : link_forGetpass)
  let subject = (status == 1 ? subject_regis : subject_forGetpass)
  let detailHtml = (status == 1 ? detailHtml_regis : detailHtml_forGetpass)
